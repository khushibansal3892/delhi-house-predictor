...
def generate_pdf_report(locality, area, bhk, bath, parking, furnish, status, trans, p_type, valuation, user_name, broker_info):
   ...
   # current_time = datetime.now().strftime("%I:%M %p")
    ...
    pdf.set_text_color(15, 23, 42)
    pdf.cell(0, 7, f"Prepared For:   {user_name if user_name else 'N/A'}", ln=True)
    pdf.cell(0, 7, f"Broker / Agency:  {broker_info if broker_info else 'N/A'}", ln=True)
    
    
    current_y = pdf.get_y()
    pdf.line(10, current_y + 3, 200, current_y + 3)
    pdf.ln(12) # Section 1 ke liye safe margins
